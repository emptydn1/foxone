...
nutTop.addEventListener('click', function(){
    if(lan2 == 1){
        this.classList.add('actibtnn');
        $('.wrapTop').addClass('activeWrapTop');
        lan2 = 2;
    }
    else{
        this.classList.remove('actibtnn');
        wrap.classList.remove('activeWrapTop');
        lan2 = 1;
    }
})
...
for (let i = 0; i < buttons.length; i++) {
    buttons[i].addEventListener('click', function(){
        if(tt2 == 1){
            ...
            if(tt2 == 2){
                setInterval(function(){ autoSlide(); }, 3000);
                tt2 = 1;
            }
        }
...
var time = setInterval(function(){ autoSlide(); }, 2000);
function  autoSlide() {
    var position = 0;
